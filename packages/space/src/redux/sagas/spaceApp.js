...
export function* createDiscussionSaga({ payload }) {
  try {
    ...
    console.log('Error in createDiscussionSaga: ' + util.inspect(error));
    yield put(errorActions.addError([error], 'createDiscussionSaga'));
  }
}
...
export function* fetchRecentDiscussionsSaga() {
  try {
    ...
    console.log('Error in fetchRecentDiscussionsSaga: ' + util.inspect(error));
    yield put(errorActions.addError([error], 'fetchRecentDiscussionsSaga'));
  }
}
