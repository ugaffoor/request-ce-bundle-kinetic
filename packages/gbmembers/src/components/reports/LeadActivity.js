...
const OpenLeadCellButton = ({ cell, onSelect, label }) => {
  return (
    <a href={`/#/kapps/gbmembers/LeadDetail/${cell.getData().id}`} className="">
      {cell.getValue()}
    </a>
  );
};
...
// One expander for a lead's whole history. Opens notes, events and all four
// message tables in a single panel beneath the row, instead of six cells each
// needing its own click.
...
export class LeadsActivityReport extends Component {
  constructor(props) {
    ...
    this.allColumns = [
      ...
      },
      {
        ...
        tooltip: true,
        bottomCalc: 'count',
        formatter: reactFormatter(<OpenLeadCellButton />),
      ...
      { title: translate('Source'), field: 'source' },
      { title: translate('Source Reference 1'), field: 'sourceReference1' },
      { title: translate('Source Reference 2'), field: 'sourceReference2' },
      { title: translate('Kids/Adults'), field: 'sourceReference3' },
      { title: translate('Source Reference 4'), field: 'sourceReference4' },
      { title: translate('Source Reference 5'), field: 'sourceReference5' },
      {
        title: 'Reminder Date',
        field: 'reminderDate',
        sorter: function(a, b, aRow, bRow, column, dir, sorterParams) {
    ...
    this.hiddenColumns = [
      ...
      { label: translate('Source'), value: 'source' },
      { label: translate('Source Reference 1'), value: 'sourceReference1' },
      { label: translate('Source Reference 2'), value: 'sourceReference2' },
      { label: translate('Kids/Adults'), value: 'sourceReference3' },
      { label: translate('Source Reference 4'), value: 'sourceReference4' },
      { label: translate('Source Reference 5'), value: 'sourceReference5' },
  ...
  attachGridWrapper = element => {
    ...
    this.gridResizeObserver = new ResizeObserver(entries => {
      ...
      this.gridRedrawFrame = window.requestAnimationFrame(() => {
        const grid =
          this.leadsActivityGridref && this.leadsActivityGridref.current;
        try {
          if (grid && grid.redraw) {
            grid.redraw(true);
          }
  ...
  hideColumns = () => {
    if (this.state.hiddenColumns && this.leadsActivityGridref) {
      this.state.hiddenColumns.forEach(column => {
        this.leadsActivityGridref.current.hideColumn(column.value);
      });
      this.leadsActivityGridref.current.redraw();
    }
  };
  ...
  removeFilter = (e, cell) => {
    ...
    if (this.state.filters && this.state.filters.length > 0) {
      if (dataType === 'date') {
        this.leadsActivityGridref.current.removeFilter(
          this.dateFilter,
      ...
      if (dataType === 'date') {
        this.leadsActivityGridref.current.clearFilter(
          this.dateFilter,
          this.filterIds[filterId],
        );
      } else if (
  ...
  addFilter() {
    ...
    if (dataType === 'date') {
      ...
      this.setState(
        ...
        function() {
          if (this.state.filters && this.state.filters.length > 0) {
            this.leadsActivityGridref.current.addFilter(
              this.dateFilter,
              filterParams,
            );
          } else {
            this.leadsActivityGridref.current.setFilter(
              this.dateFilter,
              filterParams,
            );
          }
  ...
  handlePreferenceChange = event => {
    ...
    if (!event.target.value) {
      ...
      columns.push(this.allColumns.find(column => column.field === 'phone'));
    ...
    this.setState({
      ...
      selectedColumns: filterColumns.filter(
        column =>
          !preference['Hidden Columns'].some(elm => elm.value === column.value),
      ),
      hiddenColumns: filterColumns.filter(column =>
        preference['Hidden Columns'].some(elm => elm.value === column.value),
      ),
      filters: filters,
      key: Math.random(),
    ...
    setTimeout(function() {
      compThis.leadsActivityGridref.current.clearFilter();
  ...
  toggleGroupActivity = () => {
    ...
    const grid = this.leadsActivityGridref && this.leadsActivityGridref.current;
    if (grid) {
      if (groupActivity) {
  ...
  /**
   * Opens every activity type for one lead in a single panel beneath the row
   * -- notes, events, and the four message tables -- so their whole history
   * is one click rather than six. Reuses the same fetches and column
  ...
  handleAllActivityClick = (that, cell) => {
    ...
    // Events.
    const leadItem = this.props.leads.find(l => l.id === leadId);
    const eventsTarget = section('Events');
    this.fetchDatastoreData(
      leadId,
      journey_events_url + '&q=values[Record ID]="' + leadId + '"',
      leadItem,
      this.props.triggers,
    ).then(events => {
      ReactDOM.render(
        <LeadEvents events={events} leadItem={leadItem} />,
        eventsTarget,
      );
    });
    ...
    [
      ...
      const url =
        ...
        type +
        '"';
      this.fetchData(leadId, url).then(rows => {
  ...
  downLoadTableAsCsv() {
    this.leadsActivityGridref.current.download(
      'csv',
  ...
  deleteReportPreference = () => {
    ...
    this.props.updatePreferences('Lead Activity Report', leadsActivityReport);
    var columns = [];
    columns.push(this.allColumns[0]);
    columns.push(this.allColumns.find(column => column.field === 'name'));
    columns.push(this.allColumns.find(column => column.field === 'phone'));
  ...
  render() {
    const recordCount = (this.state.activityData || []).length;
    const options = {
      ...
      <span className="reportContent">
              ...
              <div className="col-md-12">
                ...
                {this.state.deleteLeadsModal && (
                  <DeleteLeadsModalContainer
                    ...
                    leadRows={this.leadsActivityGridref.current.rowManager.activeRows}
                  /> */}
              </div>
            </div>
          </div>
        ...
        >
          <ReactTabulator
            ...
            onRef={ref => (this.leadsActivityGridref = ref)}
            layout="fitColumns"
          />
        </div>
      </span>
